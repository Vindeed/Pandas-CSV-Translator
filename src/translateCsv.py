...
column_pairs = {
    'ItemNameInternal': 'Name*',
    'ItemStatus': 'isGMActive',
    'DescCust': 'Long Description*',
    'SupplierRrp': 'OriginalPrice*',
    'TaxCategory': 'vatRate',
    'DishType': 'Meal Type*',
    'Cuisine': 'Cuisines Tag*',
    'Categories': 'Meal Time*',
    'ServingTemperature': 'isHOT*',
    'SizeMinQty': 'GM min quantity'
}
...
def map_existing_columns(input_data, output_data, column_pairs):
...
def map_and_concatenate_flavour_and_name(input_data, output_data):
...
def process_output_columns(output_data):
...
def calculate_incl_tax_price(output_data):
...
def set_default_values(output_data):
...
def process_allergens(output_data):
...
def process_dietaries(output_data):
...
map_existing_columns(input_data, output_data, column_pairs)
map_and_concatenate_flavour_and_name(input_data, output_data)
process_output_columns(output_data)
calculate_incl_tax_price(output_data)
set_default_values(output_data)

process_allergens(output_data)
process_dietaries(output_data)
...
# Removing rows that are not Active
